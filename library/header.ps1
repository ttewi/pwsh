...
﻿#& header
    ...
    <#
    $headers.colors=@{black='#1e1e1e';darkblue='#0037da';darkgreen='#13a10e';darkcyan='#3a96dd';darkred='#c50f1f';
    darkmagenta='#800080';darkyellow='#cc7703';gray='#cdd6f4';darkgray='#767676';blue='#3b78ff';green='#16c60c';
    cyan='#61d6d6';red='#e74856';magenta='#b4009e';yellow='#f9f1a5';white='#eeedf0';reset="$e[0m"};

    if($headers.extracolors-eq$true) {
        $k=$headers.colors;$k.pink='#ffa0d2'
        add-type -assemblyname 'system.drawing'
        $t=[drawing.color]

        foreach($h in $t.declaredproperties[1..(152-12)]){
            $n=$h.name
            if($k[$n]){continue}
            $c=$t::fromname($n)
            $k[$n]='#{0:x2}{1:x2}{2:x2}'-f$c.r,$c.g,$c.b
        }
    }
    #>
...
"<paleturquoise>(paleturquoise) bwaa".color()

"\n<bananacream>(bananacream) banana cream :3".color()

#$json.stringify($headers.colors)
